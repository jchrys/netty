...
final class AsciiStringUtil {
    ...
    private static void unrolledToLowerCase(final byte[] src, int srcOffset,
                                            final byte[] dst, int dstOffset, final int byteCount) {
        assert byteCount >= 0 && byteCount < 8;
        int offset = 0;
        if ((byteCount & 4) != 0) {
            final int word = PlatformDependent.getInt(src, srcOffset);
        ...
        if ((byteCount & 1) != 0) {
            PlatformDependent.putByte(dst, dstOffset + offset,
                                      toLowerCase(PlatformDependent.getByte(src, srcOffset + offset)));
        }
    }
